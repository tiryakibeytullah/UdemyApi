...
namespace IdentityUserDeneme2.APIService
{
    public class CategoryAPIService
    {
        ...
        public async Task<CategoryWithCoursesDto> GetWithCourseById(int id)
        {
            var response = await _httpClient.GetAsync($"categories/{id}/courses");

            if (response.IsSuccessStatusCode)
            {
                return JsonConvert.DeserializeObject<CategoryWithCoursesDto>(await response.Content.ReadAsStringAsync());
            }
            else
            {
                return null;
